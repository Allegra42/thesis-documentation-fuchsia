...
But goals for \ac{io} hardware itself and drivers are often the same or similar, e.g.\ reaching a certain degree of decoupling the exact device and the corresponding control interface in order that different devices from the same type, for example a few different mices, can be controlled in a uniform way\cite{tanenbaum-modern-operating-systems}.
...
Specific addresses are accessable and seek operations are feasible.
...
Peripheral devices in this category are very manifold.
Their types range from printers over network interfaces to sensors\cite{tanenbaum-modern-operating-systems}.
But some devices does not fit in the schema at all.
A hardware clock is also a peripheral device for example, but it just raises interrupts in cetain intervals instead of exchanging data.
...
These are basically programmable or  controlable via special purpose registers.
The purpose of such a controller is managing and abstraction the communication with the actual peripheral device and the computer it is attached on.
In common, the actual device is managed via control and state registers and in some cases data buffers on the controller.
In general, this means writing certain values into defined control registers on the controller triggers actions of the peripheral devices while reading them give a state informations but also results, e.g.\ sensor values after a measurement was triggered previously via the registers.
...
The first and also wide-spread method to interact with the peripheral device's control or data registers, mostly from \texit{character devices}, is mapping these few registers into the address space of the actual computer, i.e.\ into the main memory of the system.
This results in accessing a address, mapped to a control register of a peripheral \ac{io} device, is not longer accessing a storage location within the main memory but the control register outside.
...
The controller takes over the control and initalizes and executes the data transfer by issuing requests to the \ac{io} device instead of the \ac{cpu}.
...
- a dma configuration capability
- represents the bus mastering/DMA capability of a device, can be used for granting a device access to memory
- used within device drivers
- a bti can be used to pin memory used in a vmo  -> if a caller pins memory from a vmo, they are given device physical addresses that can be used to issue memory transactions to the vmo (provided the transaction has the correct bus transaction id)
(teisenberg)

Pinned Memory Token
- representation of a device DMA grant -> outstanding access grant to a device for performing DMA
- obtained by pinning memory with a bti object 
- it is valid for the device associated with the bti to access the memory represented by the pmt for as long as the pmt object is around 

see iommu
write to teisenberg

%Interrupt
- usermode io interrupt delivery
- allows userspace to wait on hardware interrupts
-> ask teisenberg?
