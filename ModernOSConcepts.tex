...
Both ends can fulfil both jobs but they can also been shutdown for reading or writing\cite{zircon-socket}.
...
\begin{itemize}
    ...
    \item \textit{Minimize response times} is especially important in interactive systems as a delayed reaction to an user input is more noticable than a longer runtime of an already long running background job.
...
One of the most important questions in the selection of a suitable scheduling strategy is whether it should be preemtible or not.
...
In the context of job scheduling, a non-preemptive job implies the job runs until it voluntary hands over the control back to the system, e.g.\ because it is waiting for a resource or an event or it reachs its completion\cite{mandl2014Grundkurs}.
...
If a job does not finish within such a slot, it needs to be rescheduled according a certail policy\cite{brause2017betriebssysteme}.
This makes such a system much more responsible.

The question whether a scheduling policy should be aware to priorized jobs follows directly after.
There are a lot of reasons to give priority to certain jobs, e.g.\ to create the impression of a notably reactive system by priorize \ac{ui} related jobs or to ensure the fastest possible processing of a security-critical task in a realtime system\cite{mandl2014Grundkurs}.
In general, priorized jobs are available on both, preemptive and non-preemptive policies\cite{brause2017betriebssysteme}.
...
For the intoduction to scheduling strategies within this work, the individual ones are examined on basis of a single \ac{cpu} core.
In fact do modern multicore processing systems using \ac{smp} manage an own job scheduling for each core seperately.
...
\acused{rr}(\acs{rr}) is intended for the the use in interactive multiuser systems.
...
Is the slice too small becomes the overhead caused by switching the process contexts superior, is its size choosen too long, the resulting strategy evokes on \ac{fcfs} more likely.
...
As a result a job with the priority 8 is executed prior to one with a priority of 4, even if it arrived shortly after the lower priorized one.
...
is a strategy to schedule various types of jobs with different importances.
...
For realtime systems with one or just a few job in the highest level, \textit{fcfs} could be more appropriated in certain cases.
Generally is an additional reassignment of the \ac{cpu} core as soon as a high priorized job enters its queue reasonable in realtime system using multilevel scheduling as a strategy\cite{glatz2015betriebssysteme}.
...
As a result, the comuting time used so far must be tracked along with each job.
...
A lower priorized job runned before one with a high priority.
...
But the high priorized task of H was to reset a watchdog to prevent a system reset accompanied with the loss of data\cite{glatz2015betriebssysteme},~\cite{silberschatz2009operating}.
...
If a higher priorized process or thread is waiting for a resource occupied by a less priorized, the prioity of the second job will be raised to the one of the higher priorized.
This ensures that the actual low prior job frees the resource as soon as possible and the truly high priorized task can run next.
...
\begin{itemize}
    ...
    \item as a fully preemtible kernel for desktop systems with low latency requirements\cite{lfd430},~\footnote{See also ``\texttt{man 7 sched}'' on Linux systems as an additional source for this section.}.
\end{itemize}
Older kernels before version 2.6.23 were not preemptible at all.
...
After the distruption, the previous task was resumed on the \ac{cpu} core.
...
\begin{itemize}
    \item Replacing spinlocks with preemptible \textit{rtmutexes} in the whole kernel.
    \item Implementing priority inheritance for locking mechanisms.
    \item Replace common interrupt handlers with preemptible kernel thread so that they can be scheduled and sleep\cite{lfd430}.
\end{itemize}
...
\begin{itemize}
    ...
    \item \texttt{SCHED\_RR} just as mentioned in the general section is \acl{rr} even in Linux a timeslicing version of the \ac{fifo} policy.
...
But the basic mechanims behind is also already known.
...
The stayes there until the required resource becomes available and thus its status changes to runnable again.
The job is put back to ,whenever possibe the previous \ac{cpu} core's, queue with the right prioity.
Was the job blocked before its time slice was finished, it becomes rewarded by beeing inserted at the front of the queue to resume as soon as possible\cite{zircon-scheduling}.
...
\begin{itemize}
    ...
    \item A priority boost. 
        \begin{itemize}
            ...
            \item A job is penalized with an one level downgrade if it is preempted after using up its entire time slice. In this situation, the prioity value may become negative.
        \end{itemize}
...
Such a job is allowed to run within its priority level without beein preempted until it blocks, yields or is manually rescheduled\cite{zircon-scheduling}.
...
Is is not merged into the standalone Zircon repository yet, but to the development master branch of the Zircon kernel within Fuchsia.
...
Memory managment is an essential part within an operating system.
...
In common, the memory is a hierachical system with \ac{cpu} registers at its top.
...
Current systems use a hierachical cache system itself.
...
As a result of this hierachy, there is often not enough space, even not in the main memory, to store the data of all running applications. 
...
In current systems, this is mostly realized using the mechanisms of \textit{address spaces} and \textit{virtual memory}, which already have been mentioned several times.
...
Distinct parts within this layout are protected against each other on base of such a layout, especially the system kernel itself\cite{brause2017betriebssysteme}.

But this term is also used for the memory region of applications which is effectively located within the main memory's address space.
Their layout is specified by the compiler or a runtime based on language conventions or standards.
A C application is for example divided into different sections like the program code itself, constants, static variables and initialised data, not initialised data, and dynamic sections for heap (data which is allocated at runtime) and stack\cite{mandl2014Grundkurs}.

However, the pure implementation of address spaces does not yet solve all of the needs named above.
Although the address space at its whole is now subdiveded into various regions, it is not ensured that the address space of a process within is always contiguous.
...
% - address space of programs is specified by the compiler/runtime according to conventions
    % -> they have different subspaces within their address space example for C/C++:
        % - program code
        % - constants, initialised data, static variable
        % - not initialised data (static vars too)
