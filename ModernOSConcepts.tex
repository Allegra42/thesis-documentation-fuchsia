...
Over the years, they had to fit in solving very different kinds of problems on very different kinds of hardware, which resulted in many different implementations and architectures. 
...
The idea behind being that requirements may change over some time and such a distinction makes it easy to adjust the \textit{policy} via parameters without touching the underlying \textit{mechanism}\cite{silberschatz2009operating}.
...
Thus, they influence on the whole system, including device driver development but also performance and security issues.
...
It allows slimming down the actual kernel by providing the ability to reload only the actually needed functionality dynamically and e.g.\ security patches within such an extension are possible without restarting the entire system\cite{brause2017betriebssysteme}.
...
Since the restrictions by the \ac{cpu}'s dual mode still apply for microkernel based operating systems is it not allowed for device drivers running in user mode to have direct physical access to \ac{io} ports as a consequence.
A device driver has to invoke the actual kernel to perform the needed action in substition.
But thus, the kernel is able to check the action and whether the driver is authorized to execute them.
