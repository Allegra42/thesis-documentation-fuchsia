...
% available scheduling policies -> man 7 sched
% - SCHED_FIFO
% - SCHED_RR
% - SCHED_DEADLINE
% - sched other
...
% - scheduler is an evolution of the LK scheduler (minimal sched)
%
% - per cpu scheduling
% - each cpu has its own set of priority queues, one for each pio level in the system (currently 32) -> fifo queues
% -> Multi Level scheduling -> with time slices, each job gets the same timeslice, if not finished, it is enqueued again
%
% - if there are not jobs in any queue, the scheduler runs the idle thread
% -> runs if no other thread is active, not held in any prio queue -> used to track idle times, may used by platform implementations for a low power wait mode
%
% - if a thread/job blocks, it is removed from the scheduling queue and added to a distinct waitqueue until the resource is available
% -> then it is inserted in a cpus queue again (in the right prio), if the job was blocked before its timeslice was finished, it is added in the front of the queue to resume as soon as possible
%
%
% - the priority is calculated from 3 factors
    % - the base prio (32 level, 0 - 31)
    % - the priority boost
        % -> given when a thread in unblocked after waiting on a resource or sleeping ++1
        % -> if a job voluntary give up the control, its boost is decremented (--) (wont go negative)
        % -> when a thread is preempted and has used up its entire time slice, its boost is decremented by one -> may go negative
    % - priority inheritance, if the thread controls a resource another with a higher prio needs (and blocks the other one), it is temporary boosted up to the prio of the higher thread -> see prio inversion
%
    % effective prio : the inherit one or the base plus boost, when the prio changes the scheduler will move a job to another queue and reschedule the cpu
%
% -> serve interactive tasks quickly -> ui tasks do little work, are often blocked by user events -> get a priority boost from unblocking
% background processes get a priority penality for using the entire timeslice
%
%
% - zircon supports also special real time threads (THREAD\_FLAG\_REAL\_TIME)
% -> they are allowed to run without preemption until they block. yield or manually reschedule
