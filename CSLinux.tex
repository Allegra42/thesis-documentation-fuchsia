...
In common, the \texttt{init()} function itself and corresponding init data are specially marked to be discarded after initialization.
While, especially the \texttt{exit()} to clean up initializations is not needed for built-in drivers because they are not unloaded at all\cite{lfd430}.
...
This will add and initialize the module but neigher the driver is initialized and ready nor is a device connected at this point.
...
- driver job: provide a uniform interface to a particular device, hiding details specific to the device's implementation


\subsubsection*{Device Model}
%General
\cite{zircon-devicemodel}
- drivers are compiled to ELF shared libraries (DSOs)
- loaded into the device host (devhost) process
- the device manager process (devmgr) contains the device coordinator 
    -> keeps track of drivers and devices, manages discovery, creation and direction of device host processes 
    -> maintains the device filesystem devfs -> mechanism through which userspace services and applications (constraines by their namespaces) gain access to devices

- device coordinator:
    - views devices as part of a single unified tree
    - branches (and subbranches) of the tree consist of some number of devices within a device host process
    -> how the tree is divided into dev host processes is based on a system policy for isolating drivers for security or stability reasons and co-location drivers for performance reasons
    (current: each device that is able to represent a physical bus master and corresponding children are placed into a seperate devhost process -> will evolve)

    \cite{zircon-ddk-gettingstarted}
    - linux -> everything in the kernel lives in the same address space -> same process
    -> fault isolation & exploitation -> bad driver can take out the entire kernel -> because of same address space and privileged access to all kernel mem and resources
    - put all drivers in an own process -> used by some microkernel systems -> much context switches
...
%Protocols
- devices may implement protocols -> C ABIs used by child devices to interact with parent devices in device specific manner (e.g. PCI protocol, usb proto, block core proto, ethermac proto, \ldots)
- protocols are usually in process interactions between devices in the same devhost, but may be rpc to a higher devhost (via a proxy)

\cite{zircon-ddk-gettingstarted}
- a protocol is a strict interface definition 
-> driver must provide a defined set of functions defined for this protocol 
-> these functions are common to all devices implementing the protocol
% not a distinction into char/block/network -> rather to protocol device types

%Interfaces
- devices may implement interfaces -> RPC protocols that clients (services, user apps) use
- base device interface: posix style open/close/read/write/ioctl io
- FIDL as interface

%Classes
- classes represent a promise that a device implements an interface or protocol
- devices exist in device filesystem unter a topological path e.g. /dev/class/classname\ldots
- current: names within the class directory are numbers -> change in future

\subsubsection*{Driver Lifecycle}
\cite{zircon-devicemodel}

%driver lifecycle
- drivers are loaded into devhost process when it is determined they are needed
    -> done by the binding program -> description of what a driver can bind to 
    -> located in an ELF NOTE section in the binary
    -> can be inspected by the device coordinator without fully load the driver into its process
    -> bind description contains a structure pointer containing pointers to init create and release methods

    - within devhost process, a device exists as a tree of zx_device_t structures

- init
    - invoked when a driver is loaded into a device host process and allows for any global initialization
    - typically none is required
    - if the method is implemented and failes, driver load fail

- bind
    - invoked to offer the driver a device to bind to
    - device is one that matches the bind rules the driver has published
    - if bind succeeds, the driver MUST create a new device and add it as a child of the device into the bind method

    - device_add() -> creates a new device, adding it as a child to the provided parent device -> must either be the device passed to bind or another device which has been created by the same device driver
    - newly created device is added to the global device filesystem -> maintained by the device coordinator
    - can be created with device invisible flag -> device is not accessible in devfs until make visible is called 
    -> used of long initialization times

    - visible added device -> device ops (read/write/fidl) may be called by the device host

    \cite{zircon-ddk-gettingstarted}
    tasks in bind:
    - initialize the driver:
        - init the device
        - setup interfaces to the driver
        - publishing a/more child/s of the device


- create 
    - invoked for platform/system bus drivers or proxy drivers -> not required for the vast majority of drivers

- release
    - invoked before the driver is unloaded, after all devices it may have created in bind and elsewhere have been destroyed -> currently never invoked 
    -> once a driver is loaded it remains loaded for the life of a device host process

    only called after the creating driver has called device_remove on the device 
    -> close all open instances of the device, remove and release all children

    -> called if a parent detects the device is removed, e.g. usb -> calls device_remove
    -> the childrens unbind method is called because the parent device is removed 
    -> unbind removes the interface that was created via device_add
    -> call release on the children

- unbind
    - if there is still work in progress when unbind is called by the parent device, the child continues
    - parent should ensure the device is not working anymore to avoid errors
