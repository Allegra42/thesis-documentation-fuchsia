...
But it is error-prone and does not even allow Linux to perform the basic checks, e.g.\ if the transfered size matches the definition for these calls.
...
The way used is pictured in listing~\ref{lst:ioctldefs}.
...
Each \ac{io} control definition is made from a \textit{type}, a \textit{number} and, if data is transfered, from the \textit{size} of the transfered data as well. 
...
In this case, the template definition of the struct is passed as a size to the macro as well.
In cases the transmitted value is a single one, its type is used.
...
It does not contain an entry for a private device data structure such as \mintinline{c}{grove_t} is or any other entry which enables the restoration of this structure.
Even if the signature slightly differs for \mintinline{c}{read()} and \mintinline{c}{write()}, it is the same issue on them.
However, each time before one of them is called \mintinline{c}{open()} must be invoked as the user must open the file descriptor which represents the device.
If it is not explicitly implemented by a driver, a default one is used.
But the fact \mintinline{c}{open()} is called can be exploited to solve this issue.
...
Its contents, especially the \mintinline{c}{i2c_client} instances for both controllers, are essential for this function.
As following preparation before stating with the implementation of the individual commands, memory is allocated for the structures \mintinline{c}{struct color_t} and \mintinline{c}{struct string_t} (see listing~\ref{lst:ioctl}, line 7 and 11).
They represent the backlight color, consisting of a red, green and blue share, and the \ac{lcd}'s content and become filled either from kernel or from user space depending on the actual command and thus, the data transfer direction.
Of course, they must be freed before leaving this function, even if it is not pictured in the according listing~\ref{lst:ioctl}.
...
As the idea of \ac{lcd} writing was already mentioned during device probing, the one of \mintinline{c}{ioctl()} shall focus on setting the \ac{rgb} backlight respectively returning its state to the user.
It is obvious that the setting of the \ac{lcd}'s content is not exactly the same as it was during \mintinline{c}{probe()}.
There, a static string was set which already contained the encoded target register on the controller.
The \mintinline{c}{ioctl()} implementation receives user defined strings and thus, string operations to do a respective encoding.
For the now considered \ac{rgb} backlight device, the implementation is much more straight forward.
Its first command defined in the concept and the according Linux header is \mintinline{c}{GROVE_SET_COLOR}.
...
Using it without further ado is possibly dangerous.
Instead, the buffer must be transfered to the kernel space, more precisely into the buffer \mintinline{c}{color_t color} which was allocated at beginning (see listing~\ref{lst:ioctl}, line 7).
The \mintinline{c}{ioctl()} header definition specified the size of this structure as transfer size of this call.
...
In cases the transfer failed, it is neccessarily to unlock the mutex and leave the entire \mintinline{c}{ioctl()} implementation properly.

On a successful transfer, the received data is used to set the color on the actual device.
Similar to the \ac{lcd}'s setup known from device probing, a structure consiting from the target registers address and the value to set is defined and an array is made from it.
Each \ac{rgb} color share is set within an own register and thus, as an own entry within the array.
The \ac{i2c} transfer is, again done as known from \mintinline{c}{probe()}.
...
The \ac{ioctl()} call argument \texttt{arg} is interpreted as an address in userspace again, but this time as a target address, while the \mintinline{struct color_t} which is stored as a part of the instance data acts as source.
