...
\begin{itemize}
    ...
    \item do the connection between applications and certain peripherals, 
...
In order to avoid re-developing or duplicating driver parts that remain the same for devices of the same type are device drivers often modeled and implemented as a hierarchical or layered model.
...
In order to obtain the necessary rights are two systemcalls needed: \texttt{iopl()} to change the privileged level and \texttt{ioperm()} to set the \ac{io} port permissions.
...
However, the module has to be built to exactly the same kernel as running in order to be loaded.
...
Writing such a driver as a module which needs to be read from a hard drive to be loaded into the system is not realizable accordingly\cite{quade2016Linux}.
...
If there is no such behavior a device, it is preferred to implement only the meaningful operations, leave the default behavior for the others one and switch to a better suited interface for the device.
...
However, the preferred way is to utilize the macros Linux provides to define beside the number and the argument types also the size of the transferred data and their transfer direction\cite{quade2016Linux}.
...
In common, the \texttt{init()} function itself and corresponding init data are specially marked to be discarded after initialization, while especially the \texttt{exit()} to clean up initialization is not needed for built-in drivers because they are not unloaded at all\cite{lfd430}.
...
As a consequence, less boilerplate code, as it e.g.\ needed for initialization must be written.
...
Thus, both variants, the two driver version which is also usable on x86\_64, but also the platform device way, are interesting for this work and should be considered.
...
As pictured in figure~\ref{pic:linux-lifecycle} is a driver's implementation of probe roughly divisible into two parts: the driver's own setup including its registration within the system and the device specific initialization.
...
As defined in the general driver concept in section~\ref{sec:cs-driver-concept}} should the driver be accessible via \ac{io}-controls and thus, the struct contains a function pointer to its implementation.
