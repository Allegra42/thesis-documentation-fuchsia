...
Thus, the resulting idea is to measure the elapsing time during a corresponding call, for example toggeling \acp{gpio}.
A more complete performance analizis of both kernels is hardly practicable as part of this work and the corresponding results can of course not give a complete picture of the kernel performances in each situation.
...
For Zircon, the choice of development boards is pretty limited to the \textit{HiKey960}.
The previous issues with this board are neglectable in this situation.
...
Both operating systems provide internal high resolution timers with a microsecond resolution which can be used for the comparison, but particularly the quality of the Zircon timer is difficult to verify.
...
Taking the elapsed microseconds since system start right before and after the considered systemcalland subtract the start time from the end time should produce a comparable call duration.
