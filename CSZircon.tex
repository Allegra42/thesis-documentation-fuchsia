...
At this point, the driver's execution starts by calling its \mintinline{c}{bind()} function with these parent device as an argument.
...
The \mintinline{c}{message()} implementation follows always the same scheme.
...
Of course, the code pictured in listing~\ref{lst:fidlimpl}, lines 1 to 27, is idiomatic for Zircon and uses these \acp{api} which were already considered during the \mintinline{c}{bind()} implementation.
...
Is the offset zero, the as many data as possible, specific in the already mentioned \mintinline{c}{count} value, is transferred.
...
Independent from this value, the \mintinline{c}{read()} function must always return \mintinline{ZX_OK} (see listing~\ref{lst:read}, line 9).
...
A structure is used to store the tuples of register address and its matching color share parsed from the input buffer and a loop steps over these tuples to send them via \mintinline{c}{i2c_write_sync()} (see listing~\ref{lst:write}, lines 26 to 38).
...
The platform device itself is represented as a \mintinline{cpp}{PDev} name class from the namespace \mintinline{cpp}{ddk} (see listing~\ref{lst:cppheader}, line 22) while its corresponding \mintinline{pdev} object is initialized using the \textit{initializer list} of the \mintinline{cpp}{GroveDevice}.
...
If that is the case, the visibility can be switched using the call \mintinline{cpp}{DdkMakeVisible()} pictured in line 22 and make the device accessible, e.g.\ via the \ac{fidl} calls.
