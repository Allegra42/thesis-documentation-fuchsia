...
\begin{figure}[!h]
 \begin{minipage}{0.45\textwidth}
  ...
  \end{minted}
  \captionof{listing}{Allocation of an I²C device in a Zircon Platform Device Driver (C)}
 ...
 \begin{minipage}{0.45\textwidth}
  ...
  \end{minted}
  \captionof{listing}{Allocation of an I²C device in a common Zircon Device Driver (C, RGB)}
...
The data to be read is transfered using the \mintinline{c}{snprintf()} function in line 4 with a defined format.
In point of fact, the resulting string should be prepared with this call.
However, the maximum size to copy is specified in the argument \mintinline{c}{count} and the user buffer is accessable without further ado, thus the call \mintinline{c}{snprintf()} can execute the transfer without any additional steps (see listing~\ref{lst:read}, line 4).
