...
Listing~\ref{lst:cppheader} pictures the way the \mintinline{cpp}{ddk::Messageable} mixin is invoked for the Grove platform device C++ driver in line 2.
...
The \mintinline{cpp}{pdev} is representated as a private member variable within the \mintinline{cpp}{GroveDevice} class.
Itself, it is representated as a \mintinline{cpp}{PDev} name class from the namespace \mintinline{cpp}{ddk} (see listing~\ref{lst:cppheader}, line 22).
The \mintinline{pdev} object is initialized using the \textit{initializer list} of the \mintinline{cpp}{GroveDevice}.
...
The \mintinline{cpp}{Bind()} function pictured in listing~\ref{lst:cppbind} looks unfamiliar when moving from C driver development as well, but at a closer look, it does nothing special.
...
Looking back to the corresponding header file, it invokes the mixins, in this case \mintinline{cpp}{ddk::Messageable} for \ac{fidl}, as well as the constructor for the \mintinline{cpp}{PDev} instance.
...
It is only needed to call \mintinline{cpp}{GetI2c()} with the item location within the \ac{i2c} definition array on the \mintinline{cpp}{pdev} (see listing~\ref{lst:cppinit}, lines 6 and 10).
...
It can be switched using the call \mintinline{cpp}{DdkMakeVisible()} pictured in line 22.
...
One is the \mintinline{cpp}{fbl} implementation of C++ strings.
...
However, the \ac{i2c} write for a \mintinline{cpp}{fbl::String} is not longer that simple as before becauseits representation matches not longer the one needed for \mintinline{cpp}{WriteSync()}.
...
The \mintinline{cpp}{DdkMessage()} method is invoked as a mixin but the implementation is very much the same as in C, except the context pointer is replaced with \mintinline{cpp}{this}.
Unfortunately, this means as well the generated \mintinline{cpp}{dispatch()} call is not specific to C++ (see listing~\ref{lst:cppmessage}, lines 1 to 3).
...
However, the actual implications of C calls in a C++ driver become visible in the \ac{fidl} implementations, for example in the pictured \mintinline{ReadLcd()} (see listing~\ref{lst:cppmessage}, lines 15 to 21).
