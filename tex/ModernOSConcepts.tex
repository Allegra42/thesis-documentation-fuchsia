...
ipc as oberbegriff for synchronization and communication\cite{glatz2015betriebssysteme} -> we handled synchronization as an extra section and focus on communication between processes and threads for now.


% memory based communication

\cite{glatz2015betriebssysteme}
self-administrated communication using a shared memory region. -> synchronization via semaphores needed
    pro: speed, transparency (self-administrated)
    con: additional (manual) synchronisation needed to obtain consistent data, breaks isolation, software can not be correctly encapsulated

\cite{brause}
process intern (between threads)
process independent (on same system) via extra shared mem region
process independent (on same system) via files/resources

threads of different processes can use the same memory region in ram -> os allocates the memory and show it within the address spaces of both processes/threads
-> synchronisation mechanisms needed 

files: also needs explicit synchronisation 

% message based communication

    % data types (message, stream, packet)
    ...
    stream: theoretical unlimited, limitation for sender/receiver not visible
    packet: standardized formats (communication protocols), packets not visible for applications, fragmentated transfer, deframentation at receiver -> hidden by networking impl
...
% synchron / asynchron
    ...
    system supported communication / data exchange 
        pro: works across system boundaries, synchronization is realized in system
        con: not that efficient as memory based mechansims
 
    % synchron \cite{glatz} \cite{brause}
    synchron: receiver has to be ready to receive data (one have to wait/block for the other to become ready)
        pro: automatic synchronization, no buffer needed
        con: no independent programm flow possible -> parallelism is limited

    % asynchron
    asynchron: sender sends even if receiver is not available to receive (needs a buffer to store data temporarily, decoupled, helps to compensate speed diverences in data processing, sender only resumes to send if the previous data is fully stored in the buffer) buffer is called mailbox (also cite \cite{brause}) or message queue, message size depends on impl -> messages need an identification (name, descriptor, handle) -> message buffer is limitated in size -> buffer empty -> receiver needs to be blocked, buffer full -> sender needs to be blocked (blocking operation mode)
    non blocking operation -> call returns immediately, reports full/empty buffer -> application has to handle the situation

        pro: independent programm flow
        con: issues if buffer is filled and receiver not ready to work, transfer time is not known
    
    asynchron communication:
    % contection-oriented e.g. telephone also \cite{brause}
        receiver needs to be known, after creating a (logical) connection /channel a receiver handle is not longer needed. after transaction is finished, connection is closed -> communication may be unidirectional (just one directio-> simplex) (altering sender -> half duplex), bidirectional (full duplex)

        pro: message order is guaranteed, reliable end-to-end connection, timeout monitoring and resend
        con: connection has to be created and destroyed -> overhead for a few messages
        
        % conectionless mail or radio also \cite{brause}
        sender uses receiver handle for each transfer, no need to create a connection -> used for few datatransfers to a lot different receivers

        pro: no need to create a connection
        con: message order not guaranteed, message loss possible, receiver needs to be addressed per message
    
        % receiver addressing \cite{glatz} \cite{brause}
    - unicast -> 1:1 relationship -> a certain receiver is addressed
    - multicast -> 1:m -> a defined group of receivers is addressed
    - anycast -> 1:1..n -> send a message to a group, at least 1 receiver
    - broadcast -> 1:n -> send to all detectable receivers

    % priority
    -> same prio -> buffer with fifo
    -> else queues for each priority
