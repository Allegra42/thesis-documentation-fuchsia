...
In this section, especially the communication between different processes is to be considered.
The one between threads of the same process has previously been discussed.

\subsubsection*{Memory Based Communication}
Since the communication of threads of one process via its shared address space is already known, it is obvious to use a comparable technique between processes as well.
...
The first one is using an \textit{external shared memory region} which is requested from the operating system.
It allocated the memory in RAM and show it in the address space of both processes\cite{brause2017betriebssysteme}.
...
The second one is a variant, using files or similar resources as a communication medium.
...
\begin{itemize}
    ...
    \item \textit{Stream} can theoretically transport an unlimited amount of data. In practice, there is a limitation not visible for the sender and the receiver.
