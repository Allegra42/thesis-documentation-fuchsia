...
\end{itemize}
But there is no single scheduling strategy that fits all situations equally well.
...
One of the most important questioning in the selection of a suitable scheduling strategy is whether it should be preemtible or not.
...
In the context of job scheduling, a non-preemptive job implies the job runs until its completion.
...
% But during this timeslice, another job is not executed before the current one is finished, even if the scheduled job waits for a resource held by another one.
% The situation described is called \textit{deadlock}.
% The currently running job is blocked, waiting for a resource, but the one holding the resource
Even on modern multicore \acp{cpu} is a non-preemptive scheduling strategy not suitable for interactive and realtime operating systems, but for batch jobs.
%TODO
\ \\
\ \\
...
% Strategies
    ...
    non-preemptive -> ``run-to-completion'' -> not suitable for interactive or realtime system -> just as the ones considered in this work
    preemptive -> priority based process interruption -> Strategies / algorithms needed 
        - mostly based on timesharing / time slice methods
        - starvation -> low priority process may never become scheduled \cite{achilles}
    ...
    % Multilevel scheduling with priorities
    \cite{mandl2014Grundkurs}
    - own queues for each priority
