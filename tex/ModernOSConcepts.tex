...
In general, parallelism can take place on different levels, e.g.\ as hardware or software paralellism.
...
The following sections take a closer look at these concepts, related topics and their implementation in Linux and Zircon.


% \cite{glatz2015betriebssysteme}
% different actions at the same time -> parallel tasks
% parallel task execution -> process model, central concept of modern os
% thread model -> parallel within applications
%
% hardware paralellism (multiprocessor, specialized controller (usb, network, graphics), are able to perform (specialized) tasks parallel to main cpu
% (independent cpus and / or specialized controller working at the same time)
%
% software paralellism: execution seems to be at the same time (e.g. more than one process on a single core cpu) -> process modell -> multiprogramming -> computing time scheduling, time multiplexing
% -> illusion of paralellism (nebenläufige programmausführung) -> scheduling strategies
%
% one process with more than one  actions using the same ressources -> thread model (simplified version of processes)
%
% \cite{achilles2006betriebssysteme}
% 'a process ia a program in execution'
%
% process: memory segements (program code, data segment, stack, heap), resources (may exclusive use(dvd writer, cpu)) -> process controll block (pcb)
%
% \cite{tanenbaum-modern-operating-systems}
% process is the most central concept
% abstraction to provide (pseudo) concurrency
%
% \cite{silberschatz2009operating}
% process resources: cpu time, memory, files, io devices, \ldots
