...
Each program in execution is modeled as a seperate process which is assigned to a virtual private \ac{cpu} including \ac{cpu} registers, especially the program counter (PC) and a private address space\cite{tanenbaum-modern-operating-systems}.~\cite{glatz2015betriebssysteme}.
...
Using \textit{process chaining}, a running process starts an independent new one and destroys itself.
...
Both remains exiting and share the same environment such as program code and address space\cite{tanenbaum-modern-operating-systems}.
