...
\paragraph{Pipes}
Pipes were one of the first \ac{ipc} mechanisms in Unix-like systems.
They are also in Linux wide-spread, especially for process chaining like it is done in the command line instruction \texttt{ps -A | sort | more}.
...
As a pipe is buffered, both processes are decoupled to some degree, which is influenced by the actual buffer size.
It is a system and architecture dependent value.


ipc:
- pipes - brause2017betriebssysteme
- fifo - brause2017betriebssysteme
- socket

\cite{silberschatz2009operating} 6.8.3
...
(vmar read/write memory within a process)
