...
A mechanism to provide the resource \textit{computing time} to each process being in need of it is required accordingly.

Scheduling is generally used as an umbrella term for the actual \textit{scheduling}, i.e.\ the planning of the process execution and allocation of resources like computing time on the basis of certain criteria, and the \textit{dispatching}, i.e.\ the mechanism which provides the assignment of a process and its data to a \ac{cpu} core respectively their exchange\cite{brause2017betriebssysteme},~\cite{mandl2014Grundkurs}.
...
The mechanism of the dispatcher is already known from context switching.
Except the change of the \ac{cpu}'s operating mode, pure process switching follows the same rules.
...
Operating systems with a fixed and static scheduling list are very rare\cite{brause2017betriebssysteme}.
...
Having the common case that more processes or, more generally spoken, jobs ready to be executed, there is a need of a policy to find the next one to run.
...
This term was already marginally mentioned in the context of process and thread synchronization (see section~\ref{sec:processes-threads}).
...
Depending on the exact policy, a maximum computation time is often given nevertheless in such a case.
...
The question whether a scheduling policy should be aware to priorized jobs follows directly.
...
It is an enhancement of the so-called \textit{\ac{fcfs}} strategy used in batch-oriented systems for preemptive timesharing systems\cite{achilles2006betriebssysteme}.
\ac{rr} does not priorize jobs.
...
In contrast to \ac{fcfs} as a non-preemptive strategy for batch jobs is the maximum time a job is allowed to use the resource \ac{cpu} limited.
Does a running job not voluntary release the \ac{cpu} within its time slice, an interrupt to interrupt it is triggered.
The job loses its \ac{cpu} core, it is reassigned to the next job in the queue.
If the replaced job was not finished yet, it becomes enqueued again\cite{achilles2006betriebssysteme},~\cite{mandl2014Grundkurs}.
The size of the time slice or \textit{quantum} is the same for each process, but the difficulty in this strategy is determination of its effective size\cite{brause2017betriebssysteme}.
...
\ac{rr} as a strategy is very cheap reagarding the calculation of the next job to schedule\cite{brause2017betriebssysteme}.
...
The subsequent \ac{rr} strategy is not touched to add priority support, but the management of the additional queue requires little more management effort\cite{brause2017betriebssysteme}.
...
The lowest priority is given to expensive and long-running \textit{batch jobs}\cite{brause2017betriebssysteme},~\cite{mandl2014Grundkurs}.
In the concept of multilevel scheduling is a distinct queue for each priority level available.
...
This strategy allows some variants.
